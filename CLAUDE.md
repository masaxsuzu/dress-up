...
3. **Run `/self-review <PR>`.** Static checklist + actual preview curl via `scripts/verify-preview.sh`. The skill posts the verdict as a PR comment (`mcp__github__add_issue_comment`). Output is one of `✓ clean` / `⚠ suspicious` / `✗ broken`.
